...
class RiskSubmissionRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        return [
            ...
            'threat_ids.*' => ['integer', 'distinct', Rule::exists('threats', 'id')->where(fn ($query) => $query->where('is_active', true)->where(fn ($query) => $query->whereNull('asset_id')->orWhere('asset_id', $this->integer('asset_id'))))],
    ...
    public function after(): array
    {
        return [function (Validator $validator): void {
            ...
            $valid = Vulnerability::whereIn('id', $this->input('vulnerability_ids'))->whereHas('threats', fn ($query) => $query->whereIn('threats.id', $this->input('threat_ids')))->count();
            if ($valid !== count($this->input('vulnerability_ids'))) {
                $validator->errors()->add('vulnerability_ids', 'Kerentanan harus terkait dengan ancaman terpilih.');
            }
        }];
    }
}
