...
export async function saveAsset(data: { nama_aset: string; kategori: string; kondisi: string; deskripsi: string }, id?: number) { const response = id ? await api.put<{ data: Asset }>(`/satker/assets/${id}`, data) : await api.post<{ data: Asset }>("/satker/assets", data); return response.data.data; }
